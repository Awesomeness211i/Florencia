...
Then put whatever data into it and store it in the queue.
Then the queue would call its ExecuteCommand() function instead of having a switch statement.
In the case of lambdas have QueuableCommand contructor take an std::function that would then get called when ExecuteCommand() is called.
...
Combine the two approaches by having the interface have a GetCommandType() function that could be used within the QueueCommand() function
